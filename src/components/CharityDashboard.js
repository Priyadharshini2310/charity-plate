...
//   const [activeTab, setActiveTab] = useState("analytics");
//   const [loading, setLoading] = useState(true);
//   // initialize as object so forms/modal fields don't break when no charity exists
...
//     } catch (err) {
//       console.error("Failed to parse user from localStorage:", err, raw);
//     }

//     const token = localStorage.getItem("token");
//     console.log("Dashboard init — user:", user, "token present:", !!token);

//     // defensive role check (case-insensitive)
//     if (!user || typeof user.role !== "string" || user.role.toLowerCase() !== "charity") {
//       navigate("/login");
//       return;
//     }

//     // call fetches (they handle missing charity by opening create modal)
//     fetchCharityData();
//     fetchDonations();
//     fetchSentMessages();
//     fetchUserProfile();
...
//       processAnalyticsData();
//     } else {
//       // reset analytics if no donations
//       setAnalyticsData((prev) => ({ ...prev, monthlyTrend: [], donationTypeBreakdown: [], topDonors: [], statusDistribution: [], weeklyStats: [] }));
//     }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [donations]);
...
//       const token = localStorage.getItem("token");
//       if (!token) {
//         console.warn("No token present for fetchCharityData; redirecting to login.");
//         navigate("/login");
//         return;
//       }
...
//       const data = await response.json();
//       console.log("fetchCharityData", response.status, data);

//       if (response.ok) {
//         const normalized = data?.data ?? data;
//         setCharityData(normalized || {});
//         if (normalized?.image) setImagePreview(normalized.image);
//       } else {
//         // when profile not found, open org modal in edit mode so user can create
//         console.warn("Charity profile not found or error:", data);
//         setCharityData({}); // allow modal to show controlled empty values
//         setShowOrgProfileModal(true);
//         setOrgEditMode(true);
//       }
//     } catch (error) {
...
//       const token = localStorage.getItem("token");
//       if (!token) {
//         console.warn("No token in localStorage for fetchDonations.");
//         return;
//       }
//       const response = await fetch(
//         "https://charityplatebe.vercel.app/api/charity/donations",
...
//       const data = await response.json();
//       if (response.ok) {
//         // normalize: accept either array or { data: [...] }
//         const normalized = Array.isArray(data) ? data : data?.data ?? [];
//         setDonations(normalized);
//       } else {
//         console.error("Failed to fetch donations:", response.status, data);
//       }
//     } catch (error) {
//       console.error("Error fetching donations:", error);
//     }
...
//       if (!token || !userId) {
//         console.warn("Skipping fetchSentMessages: no token or userId", { tokenPresent: !!token, user });
//         return;
//       }
...
//       const charityDataResp = await charityResponse.json();
//       console.log("charity/byUser response", charityResponse.status, charityDataResp);

//       if (!charityResponse.ok || !charityDataResp?.data?._id) {
//         // No charity found — prompt user to create one
//         console.warn("Charity not found for user; opening create modal.");
//         setCharityData({}); // prepare empty data for the modal form
//         setShowOrgProfileModal(true);
//         setOrgEditMode(true);
//         return;
//       }
...
//         setCharityData({ ...charityData, image: base64String });
//         setUploadingImage(false);
//         showToast("success", 'Image uploaded! Click "Save" to persist.');
//       };
//       reader.onerror = () => {
//         showToast("error", "Failed to read image file");
//         setUploadingImage(false);
...
//   // handleOrgUpdate: create (POST) when no _id, update (PUT) when _id exists
//   const handleOrgUpdate = async (e) => {
//     e.preventDefault();

//     // client-side validation
//     if (!charityData?.name || !charityData.name.toString().trim()) {
//       showToast("error", "Organization name is required");
//       return;
//     }
//     if (!charityData?.address || !charityData.address.toString().trim()) {
...
//     try {
//       const token = localStorage.getItem("token");
//       const method = charityData?._id ? "PUT" : "POST";
//       const url = "https://charityplatebe.vercel.app/api/charity/profile";
...
//       });
//       const data = await response.json();
//       console.log("handleOrgUpdate response", response.status, data);

//       if (response.ok) {
//         showToast("success", charityData?._id ? "Organization updated" : "Organization created");
//         setOrgEditMode(false);
//         setShowOrgProfileModal(false);
//         // reload data to get _id etc.
//         fetchCharityData();
//       } else {
//         // show server validation message if exists
//         const message = data?.message || data?.error || "Failed to update/create organization";
...
//                   <div>
//                     <p className="text-sm text-gray-600 font-medium">Total Donations</p>
//                     <p className="text-2xl font-bold text-gray-900">₹{((charityData?.totalDonationsReceived || 0).toLocaleString?.() ?? charityData?.totalDonationsReceived) || 0}</p>
//                   </div>
//                 </div>
//               </div>
...
//                   <div>
//                     <p className="text-sm text-gray-600 font-medium">Total Plates</p>
//                     <p className="text-2xl font-bold text-gray-900">{((charityData?.totalPlatesReceived || 0).toLocaleString?.() ?? charityData?.totalPlatesReceived) || 0}</p>
//                   </div>
//                 </div>
//               </div>
...
//                   )}
//                 </div>
//                 <h3 className="text-2xl font-bold text-gray-900 mb-1">{charityData.name || "Create your Organization"}</h3>
//                 <p className="text-sm text-gray-500 capitalize">{charityData.type === "both" ? "Food & Money Donations" : `${charityData.type || "charity"} Donations`}</p>
//                 <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-2 ${charityData.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{charityData.isActive ? "Active" : "Inactive"}</span>
//               </div>
...
const CharityDashboard = () => {
  ...
  const fetchCharityData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Charity profile doesn't exist yet
      ...
      console.error("Error fetching charity data:", error);
      // If error, assume charity doesn't exist and show setup
      setShowSetupModal(true);
    } finally {
      setLoading(false);
    }
  ...
  const handleSetupSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        showToast("success", "Charity profile created successfully!");
      ...
      showToast("error", err.message || "Failed to create charity profile");
    }
  };

  // Show setup modal if charity profile doesn't exist
  if (showSetupModal && !charityData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-pink-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-8">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Welcome to CharityConnect!</h2>
            <p className="text-gray-600">Let's set up your charity profile to get started</p>
          </div>
          ...
          <form onSubmit={handleSetupSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSetupData({ ...setupData, contactPhone: e.target.value })}
                className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent"
            ...
            <div>
              ...
              <select
                ...
                required
              >
                <option value="both">Both Food & Money</option>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="number"
                  value={setupData.distance}
                  onChange={(e) => setSetupData({ ...setupData, distance: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent"
                  placeholder="0"
                  min="0"
                  required
              ...
              <div>
                ...
                <input
                  type="number"
                  value={setupData.pricePerPlate}
                  onChange={(e) => setSetupData({ ...setupData, pricePerPlate: e.target.value })}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-transparent"
                  placeholder="0"
                  min="0"
                  required
            ...
            <button
              type="submit"
              className="w-full px-6 py-4 bg-gradient-to-r from-sky-500 to-sky-600 text-white rounded-lg hover:from-sky-600 hover:to-sky-700 transition-all shadow-lg font-medium text-lg"
            >
...
>
  ...
  <span className="text-gray-700">View My Card</span>
      ...
      {showOrgProfileModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          {!charityData ? (
            // Setup form when no charity data exists
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-8 animate-scale-in">
              <div className="text-center mb-8">
                <div className="w-20 h-20 bg-gradient-to-br from-sky-500 to-sky-600 rounded-full flex items-center justify-center mx-auto mb-4">
                  <IoHeart className="w-12 h-12 text-white" />
                </div>
                <h2 className="text-3xl font-bold text-gray-900 mb-2">Create Organization Profile</h2>
                <p className="text-gray-600">Set up your charity organization details</p>
              </div>
              ...
              <form onSubmit={handleSetupSubmit} className="space-y-4">
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    type="button"
                    onClick={() => setShowOrgProfileModal(false)}
                    className="flex-1 px-4 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 px-6 py-4 bg-gradient-to-r from-sky-500 to-sky-600 text-white rounded-lg hover:from-sky-600 hover:to-sky-700 transition-all shadow-lg font-medium text-lg"
                  >
                    Create Profile
                  </button>
                </div>
            ...
            // View/Edit form when charity data exists
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-scale-in">
            <div className="p-8">
              <div className="text-center mb-6">
                <div className="w-24 h-24 mx-auto mb-4 rounded-xl border-2 border-gray-200 overflow-hidden bg-gray-50">
                  {charityData.image ? (
                    <img src={charityData.image} alt="Organization Logo" className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center">
                      <IoHeart className="w-12 h-12 text-gray-300" />
                    </div>
