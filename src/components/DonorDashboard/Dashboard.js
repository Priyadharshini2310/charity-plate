...
import logoicon from '../../images/LogoIcon.png';
import logotext from '../../images/NameLogo.png';
import { FaClock, FaDownload, FaHome, FaUtensils, FaUtensilSpoon } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  const getImpactData = () => {
    ...
    return [
      { category: "Meals Served", value: totalPlates, icon: <FaUtensils className="w-12 h-12 mx-auto text-sky-600"/> },
      { category: "Charities Supported", value: uniqueCharities.size, icon: <FaHome className="w-12 h-12 mx-auto text-sky-600"/> },
      { category: "Impact Hours", value: totalPlates * 2, icon: <FaClock className="w-12 h-12 mx-auto text-sky-600"/> },
    ];
  };
  ...
  const AnalyticsView = () => {
    ...
    return (
      <div className="space-y-6" ref={mainRef}>
        ...
        <div className="bg-gradient-to-br from-white-50 via-white to-sky-50 rounded-xl shadow-lg p-8 text-black">
          <div className="flex items-center gap-3 mb-6">
            <Target className="w-8 h-8" />
            <h3 className="text-2xl font-bold">Your Real-World Impact</h3>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-6">
            {impactData.map((item, idx) => (
              <div key={idx} className="text-center bg-white bg-opacity-10 backdrop-blur-sm rounded-lg p-4 hover:bg-opacity-20 transition-all">
                <div className="text-[30px] text-center text-sky-600 mb-2">{item.icon}</div>
                <div className="text-3xl font-bold mb-1">{item.value}</div>
                <div className="text-sm opacity-90">{item.category}</div>
              </div>
            ))}
          ...
          <div className="w-20 h-20 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center">
            {charity.image ? (
              <img src={charity.image} alt={charity.name} className="w-full h-full object-cover"
                onError={(e) => { e.target.style.display = "none"; e.target.parentElement.innerHTML = `<span class="text-4xl">${<FaHome/>}<</span>`; }} />
            ) : <span className="text-4xl"><FaHome/></span>}
          </div>
          <div className="flex items-center space-x-1 bg-green-50 px-2 py-1 rounded-full">
            <Star className="w-4 h-4 text-green-600 fill-current" />
            <span className="text-sm font-semibold text-green-700">{charity.rating || 4.5}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={downloadReport} disabled={generatingReport} className={`px-4 py-2 rounded-lg font-medium transition-all flex items-center gap-2 ${generatingReport ? "bg-gray-300 text-gray-700 cursor-not-allowed" : "bg-sky-600 text-white hover:bg-sky-700"}`} title="Download report (PDF)">
                {generatingReport ? "Preparing..." : <><FaDownload /> </>}
              </button>
